...
**完整播放客户端 + 服务端形态收官**——自 v0.10.0 以来的 74 次提交，覆盖 P1–P6 全功能周期、服务端安全闭环、稳定性审计（B1–B18）与发布工程。

### Added
- **媒体库与播放内核（P1/P2）**：维度化媒体库（db v2/v3）、symphonia + cpal 播放引擎、播放历史/喜欢、队列抽屉、SMTC 媒体键与设备丢失恢复、系统托盘、`.ncm` 文件关联、单实例、自动更新、崩溃日志、首启向导。
- **队列与播放列表（P6.14–P6.24）**：可编辑队列（重排/移除且保持播放位置）、加入队列/播放下一/清空、全局搜索面板（Ctrl/Cmd+K）、随机与循环模式、多选批量入队、库内播放列表（db+命令+页面+加入列表全栈）、批量移除/取消喜欢、播放列表智能清理（去重+孤儿条目）、列表与详情排序、库/喜欢/历史的服务端+客户端+网格三级筛选。
- **浏览与详情（P6.10/P6.25–P6.27）**：艺人/专辑详情页、本地封面/艺人图/元数据回填/歌词/播放器封面、在线元数据拉取（封面，带护栏）、空库状态。
- **音频能力（P6.3/P6.5/P6.7）**：DSD 支持（DSF/DFF 扫描 + PCM 抽取播放，polyphase FIR 抽取升级替代盒式平均）、ffmpeg 兜底解码（APE/WavPack/WMA）、歌词点击跳转/偏移校准 + 睡眠定时器、播放列表封面马赛克 + 队列会话恢复、页面级代码分割 + 双包体预算。
- **工具箱与系统（P4/P5）**：CUE 分轨工具、统一安装器。
- **服务端形态（P8 收尾）**：M2 签名请求（HMAC + 时间窗 + nonce，RFC-0003 §4.2）、主开关 `MUSICFORGE_AUTH=off`（fnOS 默认关认证）、状态规范（三步确认/错误卡/稳定统计/分类筛选）。
- **安全与发布工程（P3）**：minisign 签名基础设施（脚本缺失密钥/工具时退出 0 不阻塞发布）、SBOM 随发布、生命周期日志轮转断言 + 回滚指南、16MB 协议上限可观测化 + 流式化决策记录。
- **质量与 CI**：前端质量门（ESLint + 组件测试 + 包体预算）、proptest 属性测试（8 项：任意输入不 panic / 扩展名大小写 / 策略解析往返 / 注册表完整性 / 评分确定性）、GUI 跨平台构建校验（Linux deb/AppImage + macOS dmg 随发布）、发布门禁独立 tag 触发工作流、SPlayer 设计语言侧边栏与统一面板。

### Changed
- UI 全面重构为 SPlayer 设计语言（侧边栏布局、统一去重/清洗/整理/回收站面板、库扫描重设计）。
- 页面级代码分割 + 双包体预算（perf）。

### Fixed
- **稳定性审计（B1–B18 等）**：崩溃/DoS + 路径穿越 + SQL 排序回退（5 处）、扫描不完整跳过过期清理 + 体量/并发闸 + 列表滚动复位、音频代际与计数（flush 必须 bump generation、暂停/停止/EOS 静音）、前端 stale-response 守卫 + 乐观更新回滚（12 处）、ncm 封面长度损坏（静默坏音）+ 根 ErrorBoundary 与存储守卫、插件链禁权准入/ACK 闸前置/输出出口检查、阻塞扫描与收集命令移出 UI 线程（async + spawn_blocking）、空库状态 + 冗余 COUNT 去除、`is_audio_ext` 大小写不敏感。
- 升级截图字段错误、bundle 签名入参聚合（clippy）、CI 跟踪 `docs/release.md` 缺口。

### Security
- **B15 插件安全**：加载期二进制完整性闸门、db `IN` 占位分块 + 计数一致性、信任表钉固 + 数据保留策略（默认保留 likes/play_history）、插件链禁权准入/ACK 前置/输出出口检查。
- 服务端 M2 签名请求（HMAC + 时间窗 + nonce）。
- 16MB 协议上限可观测化（越限仅告警字节/条数，内容绝不入日志）。
