...
describe("useWindowedTracks", () => {
  ...
  it("reset 后拉取首页；rowAt 命中缓存", async () => {
    ...
    await waitFor(() => expect(mockList).toHaveBeenCalledWith(200, 0));
  ...
  it("滚动到深处：按可视区间拉取对应页（不拉全量）", async () => {
    ...
    await waitFor(() => expect(mockList).toHaveBeenCalledWith(200, 200));
    await waitFor(() => expect(mockList).toHaveBeenCalledWith(200, 400));
    // 页 0 早已拉取；未请求过的页（如 offset 600）不应出现
    expect(mockList).not.toHaveBeenCalledWith(200, 600);
  });
});
