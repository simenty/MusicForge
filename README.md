...
1. 下载 `MusicForge-0.11.0-setup.exe`（约 1.3 MB）。
...
```bat
MusicForge-0.11.0-setup.exe /S
"%LOCALAPPDATA%\Programs\MusicForge\Uninstall.exe" /S
```

### 免安装版

解压 `musicforge-v0.11.0-windows-x64.zip`（约 1.8 MB）到任意目录，直接运行其中的 `musicforge-gui.exe`。
...
tar xzf musicforge-v0.11.0-x86_64-unknown-linux-musl.tar.gz
./musicforge-v0.11.0-x86_64-unknown-linux-musl/musicforge --version

# macOS（Apple Silicon）
tar xzf musicforge-v0.11.0-aarch64-apple-darwin.tar.gz
./musicforge-v0.11.0-aarch64-apple-darwin/musicforge --version
```
